...
import { removeDefaultAttribute, setDefaultAttributes } from '../../features/productEdit/editProductSlice';

const EditDefaultAttributeSelector = () => {
  const dispatch = useDispatch();
  const { selactedAttributes, default_attributes } = useSelector(
  ...
  const handleDefaultAttributeChange = (attributeId, value) => {
    if (value) {
      ...
      if (default_attributes.length > 0) {
        dispatch(removeDefaultAttribute(default_attributes[0]._id));
      }
      dispatch(setDefaultAttributes({ attributeId, value }));
    } else {
  ...
  const getCurrentDefaultAttributeId = () => {
    return default_attributes.length > 0 ? default_attributes[0]._id : null;
  };
            ...
            <select
              value={
                attribute._id === getCurrentDefaultAttributeId() 
                  ? default_attributes.find((da) => da._id === attribute._id)?.value || ''
                  : ''
              }
              onChange={(e) =>
                handleDefaultAttributeChange(attribute._id, e.target.value)
              }
...
export default EditDefaultAttributeSelector;